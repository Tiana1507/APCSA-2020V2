...
public class OuterSpace extends Canvas implements KeyListener, Runnable
{
   ...
   public void update(Graphics window)
   {
	...
	public void paint( Graphics window )
	{
		...
		//checks if aliens hit the ship.
		for (Alien alien : horde.getList()) { 
			if (ship.getX() <= (alien.getX() + alien.getWidth() + Math.abs(ship.getSpeed()))) {
				if (ship.getX() > (alien.getX() - ship.getWidth() - Math.abs(ship.getSpeed()))) {
					if (ship.getX()>=(alien.getX())) {
						if ((ship.getY()+ship.getHeight())>alien.getY()){
			...
			else if (ship.getX() < (alien.getX() + alien.getWidth() + Math.abs(ship.getSpeed()))) {
				if (ship.getX() >= (alien.getX() - ship.getWidth() - Math.abs(ship.getSpeed()))) {
			...
			else if (alien.getY()+alien.getHeight()>=600) {
				horde.stopHorde();
				ship.setSpeed(0);
				shots.clear();
				gameOver=true;
	...
	{
      System.out.println("I love Kanye3");
	}
